...
const useItems = ({professionSelected, getAuctionPrices, auctionHouseId}) => {
  ...
  //     // Procesa los items principales
  //     for (let item in Data.items) {
  //       const itemReagents = [];
        
  //       // Procesa los reagentes del item
  //       for (let reagent in Data.items[item].reagents) {
  //         const [reagentPrice, reagentNumAuctions, reagentPostQuantity, reagentHistorical] = await getItemPrices(reagent);
  ...
  //           name: Data.reagents[reagent].name,
  //           image: Data.reagents[reagent].image,
  //           price: Number(reagentPrice) || 0, // Precio en 0 si no se obtiene
  //           numAuctions: reagentNumAuctions,
  //           postQuantity: reagentPostQuantity,
  //           historical: reagentHistorical
  //         };
  //         itemReagents.push(reagentData);
  //       }
  ...
  //         name: Data.items[item].name,
  //         image: Data.items[item].image,
  //         price: Number(itemPrice) || 0, // Precio en 0 si no se obtiene
  //         units: Data.items[item].units,
  //         reagents: itemReagents,
  //         numAuctions: itemNumAuctions,
  //         postQuantity: itemPostQuantity,
  ...
  //     // Procesa los reagentes individuales
  //     for (let reagent in Data.reagents) {
  //       const [reagentPrice, reagentNumAuctions, reagentPostQuantity, reagentHistorical] = await getItemPrices(reagent);
  //       const newItem = {
  //         id: reagent,
